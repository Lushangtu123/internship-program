...
describe('videoQueryCache comment counts', () => {
  it('bumps matching video comment counts across pages', () => {
    const pages = [page([{ id: 'v_1', comments: 2 }, { id: 'v_2', comments: 5 }])];
  ...
  it('does not go below zero', () => {
    ...
    expect(withCommentCountDelta(pages, 'v_1', -1)[0].items[0].stats.comments).toBe(
      0
    );
  });
  ...
  it('sets absolute comment counts', () => {
    ...
    expect(withCommentCountAbsolute(pages, 'v_1', 4)[0].items[0].stats.comments).toBe(
      4
    );
  });
});
