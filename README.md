...
APIs are backed by a **local persistent JSON store** (`data/store.json`, seeded from `public/mock/seed.json`). Guests get an httpOnly session cookie automatically; register/login upgrades the identity. Likes and saves are per-user. Users can upload videos via `POST /api/videos/upload` — files are stored under `public/uploads/`, packaged to **HLS** (with progressive fallback), and played via native HLS or `hls.js`; a successful publish jumps the feed to the new clip (`/?v=`). The **For You** feed is ranked by engagement + freshness; **Following** shows creators you follow (with suggested creators when empty); **Saved** shows your bookmarks. Tap a creator avatar or handle to open `/creator/[id]`. Likes, comments, and follows generate in-app notifications (bell icon).
